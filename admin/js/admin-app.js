...
function checkAuth() {
  return sheets_verifyToken().then(function (r) {
    if (!r || !r.valid || !r.role) { window.location.href = 'login.html'; return; }
    currentRole = r.role;
    var allowed = { superadmin: 1, admin: 1, repair: 1, product: 1 };
    if (!allowed[currentRole]) { window.location.href = 'login.html'; return; }
    ...
    return sheets_getAll('users').then(function (users) {
      ...
      if (user && user.role && user.role !== currentRole && allowed[user.role]) {
        currentRole = user.role;
        setAdminRole(user.role);
      }
      if (a) {
        var n = ((user || r).name || (user || r).email || 'A')[0].toUpperCase();
        a.textContent = n;
      }
    });
  });
...
function filterProducts() {
  ...
  tbody.innerHTML = f.map(function (p) {
    ...
    var actions = manage
      ? '<div style="display:flex;gap:4px"><button class="btn btn-sm btn-secondary" onclick="editProduct(\'' + p.id + '\')">Edit</button><button class="btn btn-sm btn-danger" onclick="deleteProduct(\'' + p.id + '\')">Delete</button></div>'
...
function saveProduct(e) {
  ...
  var isNew = !id;
  sheets_save('products', id || null, data).then(function () {
    closeModal(); showToast(isNew ? 'Device added' : 'Device updated', 'success'); loadProducts();
...
function deleteProduct(id) {
  ...
  sheets_delete('products', id).then(function () {
    showToast('Device deleted', 'success'); loadProducts();
...
function filterRepairs() {
  ...
  tbody.innerHTML = f.map(function (r) {
    ...
    var actions = manage
      ? '<div style="display:flex;gap:4px"><button class="btn btn-sm btn-secondary" onclick="editRepair(\'' + r.id + '\')">Edit</button><button class="btn btn-sm btn-danger" onclick="deleteRepair(\'' + r.id + '\')">Delete</button></div>'
      : '<span style="color:var(--text-secondary);font-size:13px">View only</span>';
    return '<tr>' +
...
function editRepair(id) {
  if (!canManage('repairs')) { showToast('You do not have permission', 'error'); return; }
  var r = repairs.find(function (x) { return x.id === id; });
  if (!r) return;
...
function saveRepair(e) {
  ...
  var rIsNew = !id;
  sheets_save('repairs', id || null, data).then(function () {
    closeRepairModal(); showToast(rIsNew ? 'Repair added' : 'Repair updated', 'success'); loadRepairs();
...
function deleteRepair(id) {
  ...
  sheets_delete('repairs', id).then(function () {
    showToast('Repair deleted', 'success'); loadRepairs();
...
function renderUsers() {
  ...
  tbody.innerHTML = sorted.map(function (u) {
    var colors = { superadmin: '#ef4444', admin: '#f97316', repair: '#3b82f6', product: '#10b981' };
    var actions = manage
      ? '<div style="display:flex;gap:4px"><button class="btn btn-sm btn-secondary" onclick="editUser(\'' + u.id + '\')">Edit</button><button class="btn btn-sm btn-danger" onclick="deleteUser(\'' + u.id + '\')">Delete</button></div>'
      : '<span style="color:var(--text-secondary);font-size:13px">View only</span>';
    return '<tr>' +
      '<td>' + (u.email || '') + '</td>' +
      '<td>' + (u.name || '') + '</td>' +
...
function saveUser(e) {
  ...
  var uIsNew = !id;
  sheets_save('users', id || null, data).then(function () {
    closeUserModal(); showToast(uIsNew ? 'User added' : 'User updated', 'success'); loadUsers();
...
function deleteUser(id) {
  ...
  var u = users.find(function (x) { return x.id === id; });
  sheets_delete('users', id).then(function () {
