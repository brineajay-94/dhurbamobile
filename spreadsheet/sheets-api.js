...
function sheets_getAll(sheetName) {
  return fetch(SHEETS_API_URL + '?action=getAll&sheet=' + encodeURIComponent(sheetName))
    .then(function (r) { return r.json(); });
}

function sheets_getById(sheetName, id) {
  return fetch(SHEETS_API_URL + '?action=getById&sheet=' + encodeURIComponent(sheetName) + '&id=' + encodeURIComponent(id))
    .then(function (r) { return r.json(); });
}

function sheets_getSetting(key) {
  return fetch(SHEETS_API_URL + '?action=getSetting&key=' + encodeURIComponent(key))
...
function sheets_login(email, password) {
  return fetch(SHEETS_API_URL + '?action=auth&email=' + encodeURIComponent(email) + '&password=' + encodeURIComponent(password))
    .then(function (r) { return r.json(); })
    .then(function (res) {
      if (res.success) {
        setToken(res.token);
...
function sheets_verifyToken() {
  ...
  return fetch(SHEETS_API_URL + '?action=verifyToken&token=' + encodeURIComponent(token))
    .then(function (r) { return r.json(); })
